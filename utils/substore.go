...
func WarpUrls(rawURL string) []string {
	rawURL = formatTimePlaceholders(rawURL, time.Now())
	urls := make([]string, 0, 3)
	if strings.HasPrefix(rawURL, "https://raw.githubusercontent.com") && config.GlobalConfig.GithubProxy != "" {
		urls = append(urls, config.GlobalConfig.GithubProxy+rawURL)
	}
	urls = append(urls, rawURL)
	if cdnURL := githubRawToJsDelivr(rawURL); cdnURL != "" {
...
func githubRawToJsDelivr(rawURL string) string {
	...
	cdn := url.URL{
		...
		Path:     "/gh/" + owner + "/" + repo + "@" + ref + "/" + strings.Join(fileParts, "/"),
